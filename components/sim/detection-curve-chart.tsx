  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        if (!point) return '';
        return `주입 ${formatNumber(point.magnitude, 2)} ${unit}<br/>탐지 ${point.detected}/${point.injections} (재현율 ${point.recall === null ? '—' : `${formatNumber(point.recall * 100, 0)}%`})<br/>지연 중앙값 ${point.medianDelayDays === null ? '—' : `${formatNumber(point.medianDelayDays, 1)}일`}`;
      },
    },
    xAxis: { type: 'category', name: `주입 크기 (${unit})`, nameLocation: 'middle', nameGap: 28, data: curve.map((p) => formatNumber(p.magnitude, 2)), ...axisStyle },
    ...
    series: [
      {
        ...
        markLine:
          minDetectable === null
            ? undefined
            : { silent: true, symbol: 'none', lineStyle: { color: theme.warn, type: 'dashed' }, label: { color: theme.warn, formatter: '최소 탐지 크기' }, data: [{ xAxis: formatNumber(minDetectable, 2) }] },
      },
    ],
  };
}
