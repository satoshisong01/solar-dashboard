...
export function TechnicalDetails({ label, defaultOpen = false, children }: Readonly<{ label: string; defaultOpen?: boolean; children: ReactNode }>) {
  return (
    <details open={defaultOpen} className="group flex min-w-0 flex-col">
      <summary className="inline-flex cursor-pointer list-none items-center gap-1.5 rounded-md border border-rule bg-surface px-3 py-2 text-sm font-medium text-ink-2 hover:bg-sunken [&::-webkit-details-marker]:hidden">
