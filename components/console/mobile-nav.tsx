...
/** 사이드바가 돌아오는 폭. 여기까지 넓어지면 서랍은 닫는다 */
...
/**
 * lg 미만에서 사이드바를 대신하는 서랍 메뉴 (lg 이상에서는 여는 버튼이 없다).
 * 바깥 클릭·ESC·화면 이동·폭 확대로 닫고, 열려 있는 동안 포커스를 서랍 안에 가둔다.
 */
export function MobileNav({ showSim }: Readonly<{ showSim: boolean }>) {
  ...
  useEffect(() => {
    ...
    const sidebarBack = window.matchMedia(SIDEBAR_QUERY);
    const onWidthChange = () => {
      if (sidebarBack.matches) setOpen(false);
    };

    document.addEventListener('keydown', onKeyDown);
    sidebarBack.addEventListener('change', onWidthChange);
    const { body } = document;
    const previousOverflow = body.style.overflow;
    body.style.overflow = 'hidden';
    return () => {
      document.removeEventListener('keydown', onKeyDown);
      sidebarBack.removeEventListener('change', onWidthChange);
  ...
  return (
    <div className="lg:hidden">
      ...
      {/* 상단 바에 backdrop-blur가 걸려 있어 그 안에서는 position:fixed가 상단 바 상자에 갇힌다.
          서랍은 body로 내보내야 화면 전체를 덮는다. */}
      {open &&
        createPortal(
