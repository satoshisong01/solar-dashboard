...
const PARAMS_NOTE: Readonly<Record<string, string>> = {
  ...
  'tank.static_leak':
    ...
    '기준 정지 보유(최초 12개 중 6개 이상) 손실 중앙값을 편향으로 빼고, 잡음 σ = max(MAD σ, 기준 CI 반폭 중앙값/1.96, noiseFloor)로 하한을 둠(MAD만 쓰면 보유 수가 적을 때 σ 과소). ' +
    'zSigma 3·noiseFloor 0.02 kg/일·안전 심각도 4(CI 하한 > 0.5 kg/일) 조건은 그대로. ' +
    '표준오차 정정: 판정값은 최근 구간 가중 중앙값 − 기준 구간 중앙값이므로 SE = √(π/2)·σ·√(1/n_eff최근 + 1/n기준)로 고쳤다(가중 중앙값 비효율 1.2533 + 기준 창 불확실도, n_eff = (Σw)²/Σw²). ' +
    '예전 σ/√n최근은 기준 12·최근 6구간에서 실제 SE의 0.65배라 명목 3σ가 한쪽 약 2σ(2.5%) 검정이었다. 누설 0 합성 정지 구간 1000회 유의 판정 비율 2.1% → 0.8%. ' +
    'CI도 같은 SE로 내고(반폭 = max(1.96·SE, 최근 구간 부트스트랩 반폭)), 기준 편향 보정 한도도 maxBaselineSigma × √(π/2)·σ/√n기준으로 맞췄다. ' +
    '평가 영향: 오탐 198 → 14건(0.0784 → 0.0055건/자산·월), 정밀도 0.066 → 0.533, 탐지 지연 중앙값 10.5 → 5일, 크기 MAE 0.039 → 0.034, 재현율 0.667 → 0.593(작은 누설 0.005~0.05 kg/일을 놓친다). ' +
    '게이트: 오탐만 0.1 → 0.02건/자산·월로 강화(안전 카테고리를 낼 수 있는 탐지기), 최소 탐지 크기 임계는 스윕 최대(통과 보장)에서 측정값 0.15 + 스윕 한 단계 = 0.2 kg/일로 바꿨다. 스윕에 0.15·0.25 kg/일을 추가해 0.1~0.3 구간을 다시 쟀다(0.1 → 2/3, 0.15 이상 3/3).',
  'pv.soiling_rate':
  ...
  'comp.sec_rise':
    '운전 최소 이송량 minMassKg 1 → 5 kg: 1~2 kg 보충 운전(30분)은 기동·정지 에너지 때문에 비에너지가 정상 운전의 약 1.5배(약 3.0 vs 1.9 kWh/kg)라 기준 bin에 섞이면 부트스트랩 중앙값이 두 봉우리 사이를 오가 CI가 0을 넘었다(데모 밸브 마모 12%: 상승 +10.8%인데 CI −18 ~ +13%). 심각도 임계·bin 폭은 그대로. 평가 영향: 밸브 5% 3/3(129일) → 2/3(117일), 10%·20%·오탐 0은 그대로(짧은 운전을 빼 표본이 줄어 작은 상승의 CI가 넓어짐). 게이트 기준 크기 10%는 이 탐지기의 심각도 3 임계(sev3Pct 10%)와 같은 값이다. "심각도 3 = 조치 권고 대상"이므로 그 크기부터는 놓치지 않아야 한다는 뜻이고, 그보다 작은 크기는 게이트로 올리지 않고 곡선으로만 기록한다(밸브 마모 5% 재현율 2/3, 지연 중앙값 116.5일). 같은 규칙을 쓰는 탐지기: el.sec_rise 5%(sev3Pct 5), fc.blower_wear 20%(sev3Pct 20), ess.resistance_growth 40%(sev3Pct 40), el.voltage_rise·fc.voltage_decay 20 µV/h(심각도 3 임계 20), ess.capacity_fade 5%(심각도 3 임계 −5%). 속도로 주입하는 탐지기(ess.cell_imbalance 월 10 mV, pv.soiling_rate 0.1%/일, h2chain 3%/월)는 크기 자체가 아니라 평가 기간 안에 심각도 3 수준에 닿는 속도를 기준 크기로 쓴다. dq.gap_flatline 6 h는 탐지 기준(metric_def flatline_max_s)과 같고, inv.thermal_derating은 크기가 아니라 냉각팬 고장 시나리오, tank.static_leak은 최소 탐지 크기 곡선으로 대신한다.',
  'fc.blower_wear':
    '판정 규칙 변경 없음. 에어필터 막힘 판별 체크만 "기준 기간 이전 교체" → "창 안(상승 이후 포함) 교체"로 넓혀, 준공 직후 교체 기록이 없는 설비에서 늘 데이터없음이던 문제를 고쳤다(회복률 계산은 그대로). 심각도·재현율·오탐에는 영향이 없다.',
};

/** CI 축소 조합: 게이트마다 기준 크기 이상 주입이 시드마다 한 건 이상 남는 순번 */
const REDUCED_RUNS: { readonly runs: readonly number[]; readonly note: string } = {
  runs: [3, 4, 5, 6, 8, 9, 10, 11, 17],
  note:
    'P2 스윕 3~5(용량 5·7·10%, 전해조·연료전지 20·40 µV/h, 셀 불균형·데이터 품질은 가장 큰 크기) + P3 순번 1·3·4·5·6·12(순번 k = --runs 5+k): ' +
    '비에너지 5% 이상 정류기 10%·패러데이 6%·10%, 밸브 20%, 블로워 마모 10%/월·필터 25%·40%, 오염 0.1·0.2%/일, 저항 50%, 냉각팬 겨울·여름 시작, 누설 0.005~0.15 kg/일, 유량계 드리프트 4%/월. ' +
    '최소 탐지 크기 게이트(0.2 kg/일) 판정에는 재현율 0.9를 채우는 0.15 kg/일 주입이 필요해 순번 8(누설 0.25·스택 6%) 대신 순번 6을 넣었다. ' +
    '대조군 SIM-C 잡(P2·P3)은 전체와 같다. 전체 결과로 계산한 게이트 판정이 모두 전체와 같았다.',
};
