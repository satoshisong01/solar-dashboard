...
분석은 **수동 실행만** 있습니다(설계 §0). 관리자가 사이트·설비·기간을 고르면 결과를 발견사항(finding)으로만 저장하고, 리포트는 만들지 않습니다. 계산은 `createAnalysisRun`(실행 행 만들기) → `executeAnalysisRun`(아래 1~9단계) 두 단계이고, 둘을 이어 끝까지 기다리는 것이 `runAnalysis(db, { siteIds, assetIds?, from, to, requestedBy })`입니다. 로컬 확인용 CLI와 통합 테스트는 `runAnalysis`를, 콘솔의 "분석 실행" 버튼(`/desk`)은 실행 행만 만들고 `executeAnalysisRun`을 응답 뒤(`after()`)로 미룹니다 — 어느 쪽이든 같은 1~9단계를 돕니다.
...
- **분석 실행**: 사이트(여러 개)·설비(선택)·기간(최근 30/90/120일·사용자 지정)을 골라 Server Action이 실행 행(`om.analysis_run`)만 만들고 바로 돌아옵니다. 계산은 `after()`가 응답 뒤에 이어서 하므로(화면 시간 예산 4분, 페이지 `maxDuration` 300초) 분석이 도는 동안 다른 화면으로 옮겨도 계속 진행됩니다. 진행 상황은 `/api/desk/run-status`를 주기적으로 읽어 표시하고, 끝나면 결과 요약(새 발견사항·갱신·판정 불가 탐지기·소요 시간)과 최근 실행 10건을 보여 줍니다. 리포트는 만들지 않습니다.
- **종합 요약**(`lib/desk/digest/`, 화면 맨 위): 열린 발견사항을 한 덩어리로 알려 주는 2~4문장입니다. **세는 일은 엔진이, 문장은 AI가** 합니다 — 총 건수·계통별(태양광·배터리·전해조·수소 저장·압축·연료전지·데이터 품질)·급함별(바로 확인 4·5 / 이번 주 확인 3 / 지켜보기 1·2)·사이트별·새 건·다시 열린 건·가장 먼저 볼 건을 `stats.ts`가 세고, `template.ts`가 그 값만으로 틀 문장 4줄(몇 건인가 → 어느 계통인가 → 얼마나 급한가 → 지금 할 일)을 만듭니다. 인박스의 사이트 필터가 걸려 있으면 그 범위만 셉니다. 열린 건이 없으면 "지금은 확인할 이슈가 없습니다"와 마지막 분석 실행 시각을 대신 보입니다. '상세 보기'를 펼치면 계통별·급함별 목록이 나오고 각 줄에서 발견사항으로 갑니다.
...
- **쉬운 말 요약**(`lib/desk/plain/`): 전문 용어를 모르는 현장 담당자가 읽는 층입니다. 수치와 판정은 분석 엔진 값(`finding.effect`·근거 스냅샷)을 그대로 쓰고 표현만 바꿉니다. 4줄 — ① 무엇이 어떻게 됐는지(설비를 부르는 이름 + 변화, `headline.ts`: 탐지기 17종별 문장) ② 왜 믿을 만한지(같은 조건으로 고른 표본 수·조건 범위, `basis.ts`) ③ 왜 문제인지·언제까지 괜찮은지(외삽 가드를 통과한 추세만 시점으로, 아니면 영향 크기, `outlook.ts`) ④ 지금 할 일(플레이북 첫 점검 항목, `next-step.ts`).
  - ①은 근거 스냅샷 없이 인박스 행만으로 만들 수 있어 인박스·대시보드 목록의 한 줄 요약에 그대로 씁니다(추가 조회 없음). 클라이언트 컴포넌트는 배럴(`index.ts`) 대신 잎 모듈(`headline.ts`·`common.ts`)에서 가져옵니다 — 배럴은 근거 파서와 리포트 계획 규칙까지 끌어옵니다.
...
- **AI 설명**(`lib/llm/`, `/settings/ai`): 쉬운 말 4줄의 **문장만** 모델이 다시 씁니다. **판정과 수치는 언제나 분석 엔진 값**입니다.
  ...
  - 데스크 종합 요약도 같은 방식으로 다듬습니다(`digest.ts`·`digest-prompt.ts`). 보내는 것은 엔진이 센 건수와 엔진이 만든 4줄뿐이고, 검증(`validateLines`)은 쉬운 말 4줄과 같은 함수입니다 — **엔진이 세지 않은 숫자가 한 개라도 들어가면 거부**하고 틀 문장으로 되돌립니다(여러 건을 묶은 요약이라 방향 단어 검사만 건너뜁니다). 저장은 범위 단위입니다(`om.finding_digest`, `PRIMARY KEY(scope_key, findings_hash)`): `scope_key`는 'ALL' 또는 사이트 코드, `findings_hash`는 건수·id·상태·심각도 + 프롬프트 버전 + 틀 문장의 안정 해시라 발견사항이 하나라도 바뀌거나 문장 규칙을 고치면 새로 만듭니다.
...
- **지도**(플릿 `/fleet` 지도 보기·대시보드 지도 카드, `lib/data/map-status.ts`·`lib/data/site-map.ts`·`components/map/`): 발전소 위치에 상태색 마커를 찍습니다. 색은 열린 발견사항만 봅니다 — 안전 발견사항(안전 카테고리·심각도 4 이상)이나 심각도 4~5면 **바로 확인**(빨강), 2~3이면 **주의**(주황), 활성 게이트웨이의 마지막 수신이 안전감시 공백 기준(`SAFETY_SILENCE_MINUTES`, 기본 10분)을 넘었거나 수신 기록이 없으면 **수신 없음**(회색), 그 밖은 **정상**(초록). 심각도 1(관찰)은 플릿 매트릭스와 같이 수준을 올리지 않고, 통신이 끊겨도 이미 찾아 둔 위험이 먼저라 바로 확인·주의가 수신 없음을 이깁니다. 색만으로 구분하지 않습니다(마커·목록·배지에 아이콘과 글자를 함께 둡니다). 사이트 상태는 LATERAL 서브쿼리 한 번으로 읽습니다(사이트 수만큼 왕복하지 않음).
  ...
  - 마커 라벨이 서로 겹치면 급한 것부터 자리를 잡고 겹치는 라벨은 접습니다(`components/map/label-overlap.ts`, 지도가 멈출 때마다 투영 좌표로 계산). 대시보드 카드는 정상인 곳과 접힌 곳을 작은 점으로 줄입니다(수신 없음은 볼 수 없다는 문제라 그대로 크게 둡니다).
...
`tests/e2e/p3-chain.spec.ts`도 한 흐름입니다(serial): SIM-B 2026-03-01부터 21일 분석 → 저장용기 누설 안전 발견사항(심각도 4)·사이트 단위 물질수지 발견사항 → 대시보드 안전 발견사항 배너 → 누설 워크스페이스(안전 배너·정지 보유 구간 표·곡선) → 사이트 체인 원장 섹션(Sankey 2개·잔차 막대·PV 미활용 막대) → 탐지 준비도 매트릭스·CSV 내려받기 → 탐지기 설정 새 버전·범위 밖 값 거부·재분석 근거의 설정 버전·이전 버전 재활성 → 리포트(요약 맨 앞 안전 블록·방향 단어 편집 검증 문제·승인·인쇄 원장 절) → 압축기 밸브 교체 조치 등록·검증만 실행으로 개선 확인 → 비로그인 차단 → 분석이 리포트를 만들지 않음. 적재 기간 21일은 물질수지 탐지기 기본 기준 14일 + 최근 7일이고, 누설은 14일째(기준·최근 경계)부터 넣습니다.
