...
- **쉬운 말 요약**(`lib/desk/plain/`): 전문 용어를 모르는 현장 담당자가 읽는 층입니다. 수치와 판정은 분석 엔진 값(`finding.effect`·근거 스냅샷)을 그대로 쓰고 표현만 바꿉니다. 4줄 — ① 무엇이 어떻게 됐는지(설비를 부르는 이름 + 변화, `headline.ts`: 탐지기 14종별 문장) ② 왜 믿을 만한지(같은 조건으로 고른 표본 수·조건 범위, `basis.ts`) ③ 왜 문제인지·언제까지 괜찮은지(외삽 가드를 통과한 추세만 시점으로, 아니면 영향 크기, `outlook.ts`) ④ 지금 할 일(플레이북 첫 점검 항목, `next-step.ts`).
