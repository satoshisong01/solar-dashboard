...
- **쉬운 말 요약**(`lib/desk/plain/`): 전문 용어를 모르는 현장 담당자가 읽는 층입니다. 수치와 판정은 분석 엔진 값(`finding.effect`·근거 스냅샷)을 그대로 쓰고 표현만 바꿉니다. 4줄 — ① 무엇이 어떻게 됐는지(설비를 부르는 이름 + 변화, `headline.ts`: 탐지기 14종별 문장) ② 왜 믿을 만한지(같은 조건으로 고른 표본 수·조건 범위, `basis.ts`) ③ 왜 문제인지·언제까지 괜찮은지(외삽 가드를 통과한 추세만 시점으로, 아니면 영향 크기, `outlook.ts`) ④ 지금 할 일(플레이북 첫 점검 항목, `next-step.ts`).
  ...
  - 워크스페이스는 맨 위가 쉬운 요약 4줄 카드이고 효과·근거·원시 시계열·원인 판별은 '자세히 보기'로 접습니다(기본 접힘). 접힌 내용은 브라우저 찾기로 펼쳐지고 인쇄에는 `globals.css`의 `details::details-content` 규칙으로 함께 나옵니다. 원시 시계열 기간을 고른 뒤 돌아온 요청(`?series=`)은 펼친 채로 엽니다.
...
- **화면 안내와 용어집**: 콘솔 주요 9개 화면 상단에 "이 화면은 무엇을 보는 곳인가" 한 줄 안내(`lib/desk/plain/guides.ts`)를 둡니다 — 화면당 고정 문구이고 끄고 켤 수 있으며 설정에 저장하지 않습니다. `/help` 용어집 24개(`glossary.ts`)는 비유와 예시로 풀고, 본문 용어 옆 물음표(`components/ui/term-link.tsx`)가 `/help#<id>`로 갑니다. unit 테스트가 화면에 쓴 앵커가 모두 용어집에 있는지 검사합니다(죽은 링크 방지).
...
- **지도**(플릿 `/fleet` 지도 보기·오늘 화면 지도 카드, `lib/data/map-status.ts`·`lib/data/site-map.ts`·`components/map/`): 발전소 위치에 상태색 마커를 찍습니다. 색은 열린 발견사항만 봅니다 — 안전 발견사항(안전 카테고리·심각도 4 이상)이나 심각도 4~5면 **바로 확인**(빨강), 2~3이면 **주의**(주황), 활성 게이트웨이의 마지막 수신이 안전감시 공백 기준(`SAFETY_SILENCE_MINUTES`, 기본 10분)을 넘었거나 수신 기록이 없으면 **수신 없음**(회색), 그 밖은 **정상**(초록). 심각도 1(관찰)은 플릿 매트릭스와 같이 수준을 올리지 않고, 통신이 끊겨도 이미 찾아 둔 위험이 먼저라 바로 확인·주의가 수신 없음을 이깁니다. 색만으로 구분하지 않습니다(마커·목록·배지에 아이콘과 글자를 함께 둡니다). 사이트 상태는 LATERAL 서브쿼리 한 번으로 읽습니다(사이트 수만큼 왕복하지 않음).
  ...
  - `NEXT_PUBLIC_KAKAO_MAP_KEY`가 없거나 SDK를 불러오지 못하면 지도 자리에 안내만 남고 목록·상세가 화면을 맡습니다. E2E는 카카오 CDN을 막고(`playwright.config.ts`) `tests/e2e/kakao-stub.ts`로 SDK를 대신해 마커를 확인합니다.
