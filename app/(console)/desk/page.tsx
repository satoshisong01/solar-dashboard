...
import { InboxSection, InboxSkeleton, RunHistorySection, RunHistorySkeleton, RUN_HISTORY_LIMIT, RunSection, RunSkeleton } from './sections';
...
export default async function DeskPage({ searchParams }: DeskPageProps) {
  await requireAdmin();
  // 기다리지 않는다: 분석 실행과 인박스 필터가 같은 조회 결과를 나눠 쓴다
    ...
    <>
      ...
      <section id="inbox" className="scroll-mt-20">
        <Suspense fallback={<InboxSkeleton />}>
          <InboxSection options={options} searchParams={searchParams} />
        </Suspense>
      </section>
    </>
  );
