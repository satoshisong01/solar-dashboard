...
export type CapacityMetric = 'capacity_ah_anchored' | 'capacity_ah_cc' | 'capacity_ah_soc';

export interface CapacityBinView {
  /** 'C-rate bin 하한|셀온도 bin 하한' (온도 없음은 'na') */
  readonly key: string;
...
export function parseCapacityBinKey(key: string): ParsedBinKey {
  ...
  return { cRate: toNumber(c), tempC: toNumber(t) };
}
...
/** bin 키 → '0.10~0.15C · 20~25°C' */
export function capacityBinLabel(key: string, widths: BinWidths): string {
  const { cRate, tempC } = parseCapacityBinKey(key);
  const c = cRate === null ? 'C-rate 없음' : cRateRangeText(cRate, cRate + widths.cRate, widths.cRate);
