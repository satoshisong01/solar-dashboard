...
const REQUIREMENTS: readonly DetectorRequirement[] = [
  { detectorId: 'ess.capacity_fade', failureMode: 'ess.capacity_fade', assetClass: ['ess.rack'], metrics: ['batt.current', 'batt.soc', 'cell.temp.avg'], minPeriodS: 60, minHistoryDays: 30, severity: 3 },
  { detectorId: 'ess.cell_imbalance', failureMode: 'ess.cell_imbalance', assetClass: ['ess.rack'], metrics: ['batt.current', 'cell.voltage.max', 'cell.voltage.min'], minPeriodS: 60, minHistoryDays: 14, severity: 2 },
  { detectorId: 'pv.inverter_peer', failureMode: 'pv.inverter_underperformance', assetClass: ['pv.inverter'], metrics: ['ac.power', 'ac.power.limit'], minPeriodS: 300, minHistoryDays: 7, severity: 3 },
  { detectorId: 'inv.thermal_derating', failureMode: 'inv.thermal_derating', assetClass: ['pv.inverter'], metrics: ['ac.power', 'heatsink.temp'], minPeriodS: 300, minHistoryDays: 30, severity: 2 },
  { detectorId: 'dq.gap_flatline', failureMode: 'dq.data_gap_flatline', assetClass: [], metrics: [], minPeriodS: 3600, minHistoryDays: 1 },
];
...
describe('readinessMatrix 셀 상태 규칙', () => {
  ...
  it('설비 경로순 × 탐지기 입력 순서로 전체 셀을 만든다', () => {
    ...
    expect([...new Set(cells.map((c) => c.assetCode))]).toEqual(['ESS1/RACK01', 'ESS1/RACK02', 'PV1/INV01', 'PV1/INV02', 'WX1']);
    expect(cells.slice(0, 5).map((c) => c.detectorId)).toEqual(REQUIREMENTS.map((r) => r.detectorId));
  });

  it('ready · n/a · missing(누락 메트릭 목록)', () => {
    expect(cellOf(cells, 'ESS1/RACK01', 'ess.capacity_fade')).toMatchObject({ status: 'ready', missingMetrics: [], reasons: [], severity: 3 });
    expect(cellOf(cells, 'ESS1/RACK01', 'pv.inverter_peer')).toMatchObject({ status: 'n/a', missingMetrics: [], reasons: [] });
  ...
  it('partial: 완결성 < 0.9, 주기 > minPeriodS, 이력 < minHistoryDays — 사유를 모두 남긴다', () => {
  ...
  it('같은 메트릭 포인트가 여러 개면 완결성 → 주기 → 이력 순으로 가장 좋은 포인트를 쓴다', () => {
    ...
    expect(readinessCell(qualified, requirement).status).toBe('ready');
  });
  ...
  it('minCompleteness 조정·범위 검사, 요약(ready 비율은 n/a 제외)', () => {
    ...
    expect(summary).toEqual({ total: 25, applicable: 13, ready: 5, partial: 3, missing: 5, notApplicable: 12, readyRatio: 5 / 13 });
    expect(readinessSummary([]).readyRatio).toBeNull();
  });
});

describe('metricAcquisitionRanking 메트릭 확보 순위 (손계산)', () => {
  it('그 메트릭만 없어서 missing인 셀 수 내림차순, 동률은 심각도 합 → 관련 누락 셀 → 키 순', () => {
    ...
    expect(ranking).toEqual([
      ...
      // RACK01 ess.cell_imbalance(심각도 2)만 풀림, RACK02는 cell.voltage.max도 없어서 unlocks에 안 들어간다
      { metricKey: 'cell.voltage.min', unlocks: 1, severityWeight: 2, blockedCells: 2 },
      { metricKey: 'cell.voltage.max', unlocks: 0, severityWeight: 0, blockedCells: 1 },
    ]);
  });

  it('모든 기준이 같으면 메트릭 키 오름차순, missing이 없으면 빈 목록', () => {
    const base = { assetId: 1, assetCode: 'A', assetClass: 'x', failureMode: 'f', severity: 2, reasons: [] } as const;
