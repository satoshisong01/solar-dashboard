...
//    기준 구간 손실률 중앙값 = 겉보기 손실 편향(상태식·야간 냉각 온도 보정 오차). 편향은 ±maxBaselineSigma × σ/√기준 구간 수 안에서만 뺀다
//    (준공 때부터 새던 용기의 누설을 기준으로 지우지 않도록 한도를 둔다).
//    유의 = (결합 누설률 − 편향) > zSigma × σ / √n 이고 (CI 하한 − 편향) > 0.
...
export const TANK_STATIC_LEAK_PARAM_SCHEMA = z.object({
  ...
  maxBaselineSigma: numParam(D.maxBaselineSigma, { label: '기준 편향 보정 한도', unit: 'σ', min: 0, max: 10, description: '기준 구간 겉보기 손실률(상태식·온도 보정 편향)을 (이 배수 × σ ÷ √기준 구간 수) 안에서만 빼고 누설률을 냅니다. 0이면 보정하지 않습니다.' }),
...
function detect(input: TankStaticLeakInput, ctx: DetectorContext<TankStaticLeakParams>): DetectorResult {
  ...
  // 구간 사이 산포(MAD)는 기준 구간이 적으면 과소 추정될 수 있어, 구간 안 기울기 불확실도(Sen CI 반폭 ÷ 1.96)의 중앙값보다 작게 두지 않는다
  ...
  const biasLimit = (p.maxBaselineSigma * noise) / Math.sqrt(reference.length);
  const bias = Math.min(biasLimit, Math.max(-biasLimit, median(referenceLosses)));
  const leak = combine(recent) - bias;
  const rawCi = bootstrapCI(recent, combine, { iterations: p.iterations, rng: ctx.rng });
  const ci = { ...rawCi, ciLow: rawCi.ciLow - bias, ciHigh: rawCi.ciHigh - bias };
  ...
  const finding: CandidateFinding = {
    ...
    evidence: { ...holdEvidence({ reference, recent, leak, ci, noise, threshold, pctPerDay, safety, checks, eos, volumeM3, p }), baseline_bias: { kg_per_day: r(bias, 4), limit_kg_per_day: r(biasLimit, 4), reference_median_kg_per_day: r(median(referenceLosses), 4) } },
