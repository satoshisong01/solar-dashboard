...
describe('Abel–Noble 상태식 (tank.static_leak 탐지기와 같은 수식·상수)', () => {
  it('상수와 기준값을 고정한다 — 바꾸면 탐지기 쪽도 함께 바꿔야 한다', () => {
    expect(H2_ABEL_NOBLE_COVOLUME_M3_PER_KG).toBe(7.691e-3);
...
describe('hydrogenLedger', () => {
  ...
  it('탱크 하나라도 경계값이 없으면 저장 변화 null, 수소 설비가 없는 사이트는 전부 null', () => {
    ...
    expect(pvOnly.ledger).toEqual({ produced: null, fc_consumed: null, stored_delta: null, vented_est: null, residual: null, residual_pct: null, method: { produced: null, fc_consumed: null, stored_delta: null, vented: 'not_estimated' } });
