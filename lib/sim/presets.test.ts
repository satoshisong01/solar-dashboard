...
describe('EVAL_PRESET · evalRunPlans', () => {
  ...
  it('P3 순번: 스윕 크기를 순번마다 넣고, 헷갈리게 하는 고장(누설·비에너지↔유량계 드리프트, 밸브↔씰, 블로워 마모↔필터)은 같은 순번에 두지 않는다', () => {
    ...
    expect(p3.map((p) => p.magnitudes.p3?.tankLeakKgPerDay ?? null)).toEqual([0.005, 0.01, 0.02, 0.05, 0.1, 0.15, 0.2, 0.25, 0.5, null, null, null, null]);
    expect(p3.map((p) => p.magnitudes.p3?.elzSecRise?.mode ?? null)).toEqual(['rectifier', 'rectifier', 'rectifier', 'faradaic', 'faradaic', 'faradaic', 'stack', 'stack', 'stack', null, null, null, 'purge']);
    expect(p3.map((p) => p.magnitudes.p3?.flowmeterDriftPctPerMonth ?? null)).toEqual([null, null, null, null, null, null, null, null, null, 1, 2, 4, null]);
