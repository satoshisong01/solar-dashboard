...
// 데이터는 globalSetup이 만든 가상 사이트 3곳(SIM-A/B/C)이고, 열린 발견사항 수는 앞선 테스트에 따라 달라지므로
...
test.describe('지도', () => {
  ...
  test('플릿 지도에 사이트 3곳이 마커로 그려지고, 상단 요약이 수준별 건수를 보여 준다', async ({ page }) => {
    ...
    // 목록도 같은 3곳이고, 각 항목에 상태와 마지막 수신이 적혀 있다
  ...
  test('목록에서 고르면 그 발전소가 상세 패널에 열리고 수치 타일 4개와 진단 한 줄이 보인다', async ({ page }) => {
    ...
    const code = /(SIM-[A-Z])/.exec(await second.innerText())?.[1];
  ...
  test('마커를 누르면 그 사이트 화면으로 간다', async ({ page }) => {
    ...
    const code = /(SIM-[A-Z])/.exec(label)?.[1];
    expect(code).toBeTruthy();
