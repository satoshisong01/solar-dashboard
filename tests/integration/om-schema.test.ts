...
const OM_TABLES = [
  'asset', 'asset_class', 'asset_event', 'event_log', 'gateway', 'gateway_key', 'ingest_batch', 'm_1h',
  'market_daily', 'measurement', 'metric_def', 'point', 'rollup_dirty', 'site', 'unmapped_source',
];
...
describe('om 스키마 (hysol_test)', () => {
  ...
  it('P1 테이블이 모두 있다 (파티션 자식 제외)', async () => {
    const { rows } = await client.query<{ name: string }>(
