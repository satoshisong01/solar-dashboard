...
- 기준: **2026-09-16**, 브랜치 `renewal/om-console`(P3 후속 보강까지). 처음 작성 기준은 2026-09-15 커밋 `0b96129`이고, 그 뒤 바뀐 것은 §13.9에 모아 두었다.
- 이 절은 §0~§12 본문을 고치지 않고, 구현이 본문과 **달라진 곳만** 적는다. 본문과 이 절이 다르면 이 절이 현재 코드의 동작이다. §0 확정 결정은 모두 지켰다.
- §13.8은 **지금 남아 있는** 한계만, §13.9는 **해결한** 한계와 그 근거 수치를 담는다.
...
판정에 직접 영향을 주는 값만 옮겼다. 전체 파라미터의 min·max·설명은 각 탐지기의 `paramSchema`와 `/settings/detectors` 화면에 있다. 모든 탐지기의 버전은 `@1`이다. "기준 표본"은 bin별로 가장 이른 표본이고, 간격 상한을 넘는 bin은 결합에서 뺀다. 입력 요건(필수·권장 메트릭, 메트릭별 주기 상한, 최소 이력)은 `defaultParams`가 아니라 레지스트리 `requires`에 있다 — §13.6과 [데이터 계약 초안 부록 A.2](data-contract-draft.md)를 본다.

이 표의 값은 2026-09-16 코드 `defaultParams`에서 그대로 옮겼다. P3 후속 작업(§13.9)에서 바뀐 것은 `tank.static_leak`의 유의 판정 규약뿐이고(표준오차 정의), 임계값 자체는 어느 탐지기도 바꾸지 않았다.

| 탐지기 | 고장모드 · 카테고리 | 기간 · 표본 | 조건 bin | 심각도 기준 | 그 밖의 핵심값 |
|---|---|---|---|---|---|
| `dq.gap_flatline` | `dq.data_gap_flatline` · data_quality | 완결성 < 0.95 또는 결측 합계 ≥ 2 h | — | 2 고정 | 고착 구간 기준 `minFlatlineHours` 6 h(고착 후보 구간을 세는 기준은 메트릭별 `metric_def.flatline_max_s`, 일사량 2 h), 근거 포인트 10개 |
...
| `tank.static_leak` | `h2.storage_leak` · safety | 정지 보유 ≥ 4 h·샘플 ≥ 12·완결성 ≥ 0.8, 기준 12구간(최소 6), 최근 6구간(최소 4), 최근 30일 | 용기별 | CI 하한 > 0.5 kg/일 → safety 4, 유의하지만 그 미만 → performance 3 | 유의 배수 3σ — 유의 기준 = 3 × SE, **SE = √(π/2)·σ·√(1/n_eff최근 + 1/n기준)**(§13.9). CI 반폭 = max(1.96 × SE, 최근 구간 부트스트랩 반폭). 편향 보정 한도 3 × √(π/2)·σ/√n기준. 잡음 σ 하한 0.02 kg/일 · 상태식 `lemmon2008` · 하류 압력 상승 1 bar, 온도 상관 0.6 · 뱅크 교차 확인 지지 ≥ 0.5·반박 ≤ 0.2 |
...
| 규칙 | 구현 | 관련 파일 |
|---|---|---|
| 요구 조건 원천 | 탐지기 레지스트리 `requires`: 설비 종류(`assetClass`), 메트릭 목록(`metrics`), 최소 이력(`minHistoryDays`). 메트릭은 `{ key, maxPeriodS, optional? }`이고 **주기 상한은 메트릭마다 다르다** — 그 메트릭이 판정에 기여하는 가장 빠른 현상의 시간상수로 정한다(전기적 순시값 60초 `FAST_S`, 적산·온도·압력·상태·누적 카운터 300초 `SLOW_S`). `optional: true`는 **권장 메트릭**이다: 없어도 ready이고 판별 체크·조건 bin·보조 축만 줄어든다. 확보 순위 가중치용 대표 심각도는 카테고리로 정한다: safety 4, degradation·performance·availability 3, data_quality 2 | `lib/analytics/detectors/requirements.ts`, `lib/analytics/detectors/*.ts`, `lib/analytics/readiness/registry.ts` |
| 셀 상태 | · n/a: 설비 종류가 해당 없음<br>· 없음(missing): 필수 메트릭 포인트가 하나라도 없음<br>· 부분(partial): 메트릭은 모두 있으나 완결성 < 0.9(데이터 없음은 0으로 봄), 주기 > 상한, 이력 < 최소 이력 중 하나 이상<br>· 준비(ready): 그 밖<br>같은 메트릭 포인트가 여럿(한정자)이면 완결성 → 주기 → 이력 순으로 가장 좋은 포인트를 쓴다. 셀 이력은 쓰는 **필수** 포인트 이력의 최솟값이다. 주기는 메트릭마다 그 메트릭의 `maxPeriodS`로 본다. 권장 메트릭은 상태를 낮추지 않는다 — 없으면 `recommendedMissing`에만 남고(매트릭스 셀에 `*`, 툴팁·CSV에 메트릭 이름), 있으면 완결성·주기를 보지 않는다 | `lib/analytics/readiness/matrix.ts` |
| 행 구성 | · 첫 행 '(사이트 전체)': 사이트 단위 탐지기(물질수지·오염)만 판정한다. 요구 설비 종류 설비들의 포인트를 합쳐 보고, 그런 설비가 사이트에 없으면 n/a다.<br>· 설비 행: 대상 설비 종류가 같은 탐지기만 판정한다. 자기 포인트에 분석이 합쳐 쓰는 상위·형제·사이트 설비 포인트(에피소드 입력 규칙 + `READINESS_EXTRA_SOURCES`: 인버터의 기상 설비 외기 온도, 전해조 스택의 정류기 효율·전해조 퍼지 카운터)를 더한다.<br>· `dq.gap_flatline`은 포인트가 있는 모든 설비에 적용한다.<br>· 한정자 포인트(`valve.open#inlet`)는 메트릭 키만으로 요구 조건과 맞춘다 | `lib/analytics/readiness/site.ts`, `lib/analytics/pipeline/{sources,targets}.ts` |
...
| 확보 순위 | unlocks(그 메트릭 하나만 없어서 없음인 셀 수) → severityWeight(그 셀들의 심각도 합) → blockedCells(그 메트릭이 누락 목록에 있는 셀 수) → 메트릭 키 순. **필수 메트릭만** 센다 — 권장 메트릭은 없어도 탐지기가 돌아가므로 벤더 확보 순위에 올리지 않는다 | `lib/analytics/readiness/ranking.ts` |
| 화면·CSV | `/data/readiness`. 상태는 색만으로 구분하지 않고 아이콘·글자·툴팁을 함께 준다. 권장 메트릭이 빠진 셀에는 `*`와 표 아래 각주를 붙인다. CSV `/api/readiness.csv`는 기본이 매트릭스(열: … 누락 필수 메트릭 · 누락 권장 메트릭 · 부족 사유), `table=acquisition`이면 확보 순위다. UTF-8 BOM, 비로그인은 401 | `app/(console)/data/readiness/page.tsx`, `app/api/readiness.csv/route.ts`, `components/data/readiness-tables.tsx` · `6ede05b` |

### 13.7 sim:eval 게이트와 최신 수치

`lib/analytics/scorecard.json`(생성 2026-09-16, 전체 게이트 통과) 기준이다.
...
- **2026-09-16 회귀 확인:** 준비도 요구 조건·조치 효과 검증·리포트 팩 변경 뒤 축소 조합을 다시 돌려 게이트 38개 전부 통과(캐시 없이 14분 10초, 누설 오탐 0.004·정밀도 0.538·최소 탐지 크기 0.15). 이 세 변경은 탐지 판정에 관여하지 않으므로 스코어카드는 다시 만들지 않았다.
- **게이트:** 38개(P2 19 + P3 19).
...
재현율·크기오차 게이트가 어느 크기에서 판정하는지(아래에서 "기준 크기")는 임의로 고른 값이 아니다. **그 탐지기의 심각도 3 임계와 같은 값**으로 둔다. 심각도 3은 리포트에서 조치 권고 대상이 되는 경계이므로, "조치가 필요한 크기부터는 놓치지 않는다"가 게이트의 뜻이다. 그보다 작은 크기는 게이트로 올리지 않고 아래 곡선에만 기록한다 — 탐지 한계 근처의 작은 크기를 게이트로 올리면 통과를 맞추려고 임계를 낮추게 되고, 그 대가는 오탐이다. 근거는 `lib/sim/eval/p3-gates.ts` 주석과 `scorecard.json`의 `comp.sec_rise.params_note`에 같이 적혀 있다.

| 게이트 | 기준 크기 | 대응하는 심각도 3 임계 (`defaultParams`) | 최신 값 |
|---|---|---|---|
| `ess.capacity_fade.recall_5pct` · `integrated_recall_5pct` | 5% | `sev3Pct` −5% | 1 · 1 |
| `el.voltage_rise.recall_20uvh` · `rel_error_20uvh` | 20 µV/h | `sev3UvPerH` 20 | 1 · 0.003 |
| `fc.voltage_decay.rel_error_20uvh` | 20 µV/h | `sev3UvPerH` 20 | 0.034 |
| `el.sec_rise.recall_5pct` | 5% | `sev3Pct` 5% | 1 |
| `comp.sec_rise.recall_valve_10pct` | 10% | `sev3Pct` 10% | 1 |
| `fc.blower_wear.recall_20pct` | 20% | `sev3Pct` 20% | 1 |
| `ess.resistance_growth.recall_40pct` | 40% | `sev3Pct` 40% | 1 |

**속도로 주입하는 탐지기**(고장이 한 번에 나타나지 않고 계속 자라는 것)는 크기 자체가 아니라 **평가 기간 안에 심각도 3 수준에 닿는 속도**를 기준 크기로 쓴다.

| 게이트 | 기준 크기 | 심각도 3에 닿는 데 걸리는 시간 | 실측 탐지 지연 중앙값 |
|---|---|---|---|
| `ess.cell_imbalance.recall_10mv` | 월 10 mV | `deltaMvSev3` 40 mV → 약 4개월 | 74일 |
| `pv.soiling_rate.recall_0_1pct_day` | 0.1%/일 | `sev3LossPct` 누적 4% → 40일 | 45일 |
| `h2chain.mass_balance_gap.recall_drift_3pct` | 3%/월 | 잔차율 임계 `residualPct` 2% → 약 20일 (이 탐지기의 기본 심각도는 2, 누설 교차 확인이 '지지'면 3) | 34일 |

**크기 축이 다른 셋.** `dq.gap_flatline.recall_6h`의 6 h는 이 탐지기의 판정 임계(`minFlatlineHours` 6)와 같은 값이다 — 심각도 2 고정이라 심각도 3 임계가 없다(메트릭별 `metric_def.flatline_max_s`는 어떤 구간을 고착 후보로 셀지를 정할 뿐이고, 일사량은 2 h다). `inv.thermal_derating.recall_fan_failure`는 크기가 아니라 냉각팬 고장 시나리오 자체의 재현율이고(심각도는 손실률 1%·3%로 본다), `tank.static_leak`은 재현율 게이트 대신 최소 탐지 크기 곡선으로 대신한다.

**`comp.sec_rise`가 10%인 이유.** 심각도 3 임계(`sev3Pct` 10%)와 맞춘 값이고, 한 단계 아래인 5%(심각도 2 임계)를 게이트로 올리지 않은 것은 실측 재현율이 2/3·탐지 지연 중앙값 116.5일이기 때문이다. 5%를 게이트로 올리면 통과를 맞추려고 `minMassKg`(운전 최소 이송량 5 kg)을 되돌리거나 CI 조건을 느슨하게 해야 하는데, 그 두 값은 각각 짧은 보충 운전의 비에너지 두 봉우리와 오탐을 막고 있다. 5% 곡선은 아래 "크기별 탐지"에 그대로 남긴다.

**`tank.static_leak` 오탐 게이트를 0.1 → 0.02건/자산·월로 강화한 이유.** 이 탐지기만 safety 카테고리(심각도 4)를 낼 수 있고, safety finding은 오늘 화면 상단 배너로 올라가 현장 확인을 부른다. 헛경보가 잦으면 배너 자체를 믿지 않게 되므로 다른 탐지기(0.1)보다 한 자리 엄격하게 둔다. 0.02건/자산·월은 **용기 1대당 약 50개월에 헛경보 1건**이고, 용기 4대짜리 SIM-B 뱅크로 보면 사이트당 1년에 약 1건이다. 표준오차 정정(§13.9) 전 실측은 0.0784(사이트당 연 3.8건)여서 이 기준을 통과하지 못했고, 정정만으로 0.0055(사이트당 연 0.26건)가 되어 통과했다 — 탐지 임계를 느슨하게 하거나 억제 규칙을 덧붙이지 않았다.

| | 정정 전 | 정정 후 | 게이트 |
|---|---|---|---|
| 오탐 [건/자산·월] | 0.0784 (FP 198건) | **0.0055** (FP 14건) | ≤ 0.02 |
| 정밀도 | 0.066 | **0.533** | — (설명·스코어카드 `precision`에만 기록) |
| 재현율 | 0.667 | 0.593 | — |
| 최소 탐지 크기 [kg/일] | 0.1 | **0.15** | ≤ 0.2 |
| 탐지 지연 중앙값 [일] | 10.5 | **5** | — |

최소 탐지 크기 게이트도 같이 바꿨다. 예전 임계는 스윕 최대(0.5 kg/일)여서 어떤 값이 나와도 통과가 보장됐다. 스윕에 0.15·0.25 kg/일을 넣어 0.1~0.3 구간을 다시 재고(0.1 → 2/3, 0.15 이상 3/3), **측정값 0.15 + 스윕 한 단계 = 0.2 kg/일**로 고정했다. 즉 지금 값이 나빠지면 게이트가 실제로 깨진다.

**현장에서 뜻하는 바.** 시드 사이트 SIM-B의 저장뱅크는 용기 4대 × 내용적 1,850 L, 최고 450 bar다(명판 가용 180 kg). 25 °C·450 bar 만충이면 용기 1대 약 52.6 kg, 뱅크 약 210 kg이다.

| 값 | kg/일 | 만충 용기 1대(52.6 kg) 대비 | 1년 누적 | 뜻 |
|---|---|---|---|---|
| 최소 탐지 크기 | 0.15 | 약 0.29%/일 | 약 55 kg | 1년 방치하면 만충 용기 1대분이 빠져나가는 속도. 이보다 느린 누설은 이 콘솔이 정지 보유 압력만으로는 잡지 못한다 |
| 안전 심각도 4 임계 (`safetyKgPerDay`) | 0.5 | 약 0.95%/일 | 약 183 kg | 명판 가용 재고(180 kg) 전량에 해당한다 |
| 데모 SIM-B 실제 누설 (H2BANK1/TANK3) | 1.03 | 약 2%/일 | 약 377 kg | safety 심각도 4로 나온다 |

단위 주의: 위 %/일은 **만충 용기 1대 기준**이고, 뱅크 4대 합계 기준으로는 1/4(0.15 kg/일 → 약 0.07%/일)이다. 이 값들은 안전 판단 기준이 아니라 "이 분석이 얼마나 작은 누설까지 보는가"의 척도다. 현장 누설 판단·운전 정지는 가스 검지기·안전설비·현장 안전책임자 몫이다.
...
여기에는 **지금도 남아 있는** 한계만 적는다. P3 후속 작업으로 해결한 항목(저장용기 누설 표준오차, 판별 체크 입력 3개, 준비도 주기 상한, 조치 효과 검증 표본 방식, 개발 의존성 취약점)은 §13.9로 옮겼다.

| 항목 | 내용 | 현장 적용 전 확인·조치 | 근거 |
|---|---|---|---|
| SOC 기반 용량 추정의 BMS 의존 | · 휴지 앵커·CC·SOC 변화 방식은 BMS SOC 재보정 품질에 의존한다. 탐지와 조치 효과 검증이 같은 표본 방식을 쓰도록 맞췄지만(§13.9), **방식이 같아도 앵커(CV 종료) 이외는 모두 BMS SOC에 기댄다** — 그 방식으로 나온 "개선"은 SOC 재보정 변화와 구분되지 않는다. 지금 할 수 있는 것은 어떤 방식을 썼는지 근거·리포트 팩에 남기는 것까지다.<br>· 시뮬레이터 SOC는 참값이라 재보정 오차가 없어 평가 결과가 실제보다 좋게 나온다.<br>· 용량 감소가 끝난 뒤 처음 나타난 bin은 그 감소를 보지 못한다.<br>· 검증 결합은 가중치 없이 bin 중앙값 차이를 쓴다(탐지기는 휴지 앵커 표본에 1/상대분산 가중치를 쓴다) | 파일럿 BMS의 휴지 OCV 재보정 동작과 SOC 점프 빈도를 확인한다. 정기 용량시험 결과를 조치로 기록한다. 리포트의 "표본 방식" 문장을 보고 앵커 이외 방식의 결과는 보수적으로 읽는다 | `lib/analytics/detectors/ess-capacity-samples.ts`, `lib/analytics/verification/before-after.ts` |
...
| `tank.static_leak` 남는 검정 크기 차이와 작은 누설 민감도 | 표준오차 정정 뒤에도 남는 것(§13.9에 정정 내용).<br>· **명목 0.135%(3σ 한쪽) vs 실측 0.8%.** 남는 차이는 기준 12구간 MAD로 σ를 재는 표본오차다. 기준 구간 수를 늘리면 줄지만 준공 초기에 기준 확보가 늦어진다.<br>· **작은 누설은 못 본다.** 0.005~0.02 kg/일 0/3·0/3, 0.05 1/3, 0.1 2/3. 최소 탐지 크기 0.15 kg/일 = 만충 용기 1대(52.6 kg)의 약 0.29%/일(§13.7 "현장에서 뜻하는 바").<br>· 오탐 16배 감소와 맞바꾼 결과이고, 탐지 지연은 오히려 10.5 → 5일로 좋아졌다 | 기준 구간 수(12)를 사이트별로 올릴지 파일럿에서 판단한다. 0.15 kg/일보다 작은 누설은 이 분석이 아니라 가스 검지기·정기 누설 점검으로 잡는다. 현장 누설 판단·운전 정지는 가스 검지기·안전설비·현장 안전책임자 몫이다. 이 콘솔은 법정 안전설비·가스 검지기·현장 PLC 인터록 판단을 대체하지 않는다 | `lib/analytics/detectors/tank-static-leak.ts`, `lib/analytics/scorecard.json` |
| 안전 finding 심각도 하향이 조용하다 | 안전 카테고리 finding이 심각도 3으로 내려가면 오늘 화면 배너에서 그냥 빠진다. 전이 기록도 알림도 없어서, 보던 사람은 "사라졌다"만 본다. safety → performance 카테고리 전환 자체는 `73282b6`에서 갱신되도록 고쳤지만 **전이를 남기지는 않는다** | 심각도 하향 시 기록·표시 방식을 정한다(전이 이력, 배너에 '해제됨' 잔류 등) | `lib/analysis/findings.ts`, `components/today/safety-banner.tsx` · `73282b6` |
...
| 판별 체크 입력 — 남은 것 | 입력 연결은 §13.9에서 했다. 남는 것은 하나다.<br>· ~~전해조 퍼지 카운터가 카탈로그에 없다~~ → **2026-09-21 해결.** `purge.count`를 전해조 설비(`h2.elz`)에도 매핑하고 시뮬레이터가 수소측 퍼지(전하량 비례)를 센다. 비에너지 상승 고장에 퍼지 경로를 더해 판별 체크가 실제 데이터로 지지·반박을 낸다. 실사이트 GP-1은 계기가 없어 [데이터 계약 부록 B.9](data-contract-draft.md)의 요청 항목이다. **데모 DB는 다시 시드하면 새 포인트에 과거 데이터가 없어 `dq.gap_flatline`이 뜰 수 있다 — 백필과 함께 채운다.**<br>· **메모리 평가(`lib/sim/eval`)에는 교차 확인 입력을 넣지 않았다**(퍼지 입력은 2026-09-21에 넣었다). 두 체크는 판정(심각도·finding 수)에 영향이 없어 게이트에 쓰이지 않고, 넣으면 준비 결과 캐시가 무효가 되어 평가 시간만 는다. 따라서 **판별 체크의 지지·반박 정확도는 시뮬레이터로 검증되지 않았다** — 데모 DB 1건씩만 확인했다 | 판별 체크 정확도는 파일럿 기각 사유(§8 P4)로 확인한다. 교차 확인 입력도 평가에 넣을지는 평가 시간과 견줘 정한다 | `lib/analysis/aux-inputs.ts`, `lib/analysis/tank-holds.ts`, `db/seed/templates-hydrogen.ts`, `lib/sim/eval/` |
| 짧은 기준 기간과 계절 bin 이동 | · 같은 조건 비교에서 여름 bin 기준이 고장 뒤에 생기면 효과를 작게 잡는다(블로워·압축기·내부저항).<br>· `el.sec_rise` 3% 상승 0/9, 크기 MAE 3.339, 정류기 경로 판별 체크 지지 비율 0.333.<br>· 평가 곡선에 단위가 섞였다: 압축기 씰 누설(bar/일, 0/9)이 % 곡선에, 블로워 "10" 구간에 마모 %/월과 막힘 %가 함께 있다 | 1년 이상 기준을 모으기 전에는 크기보다 방향·추세로 해석한다. 설비 교체·설정 변경은 `asset_event(resets_baseline)`로 기록한다 | `lib/analytics/scorecard.json` · `ae72e31` |
| 카탈로그에 없는 메트릭 | 일 강수량(오염 복원은 맑은 날 PI 급상승으로 대신), 압축기 흡입 가스 온도(외기 온도로 대신). 전해조 퍼지 카운터는 2026-09-21에 넣었다 | 현장에서 받을 수 있으면 `metric_def`에 추가하고 탐지기 입력을 바꾼다 | `db/seed/catalog.ts`, `lib/analytics/episodes/compressor.ts` |
| 데모에 남은 권장 메트릭 누락 | 준비도 화면·CSV에 `*`로 보이는 2건이다(준비 상태 자체는 유지). COMP1 `vibration.rms` — 카탈로그에는 있으나 시드에서 일부러 매핑하지 않았다(`comp.sec_rise` 판별 체크 ③이 계속 데이터없음). ELZ1/STACK1 `purge.count` — 2026-09-21 ELZ1에 포인트를 추가했다(데이터가 쌓이면 누락 표시가 사라진다) | 벤더 협의에서 권장 메트릭 목록([부록 A.2](data-contract-draft.md))을 함께 넘긴다 | `lib/analytics/readiness/matrix.ts`, `db/seed/` |
| 준비도 — 주기 상한 값의 근거와 기준 시각 | 주기 상한을 메트릭별로 나눈 것은 §13.9에서 했다. 남는 것은 셋이다.<br>· **상한 값(60초·300초) 자체는 시뮬레이터·현장 통념 기준이고 제조사 사양으로 검증하지 않았다.** 특히 DCIR용 랙 전압·전류는 데이터 계약 초안의 권장이 2초 이하인데 요구 상한은 60초다.<br>· **완결성은 '지금'부터 30일로 계산한다.** 수신이 멈춘 데모 데이터는 날이 지날수록 완결성이 떨어진다.<br>· **`READINESS_EXTRA_SOURCES`는 사이트 안 같은 종류 첫 설비로 근사한다**(실제 로더는 형제·상위를 정확히 찾는다). 전해조·정류기가 여러 대인 사이트에서는 준비도 판정이 실제 로더와 어긋날 수 있다 | 메트릭별 상한 값은 현장 설비 사양이 오면 다시 본다. 완결성 기준 시각을 '마지막 수신'으로 바꿀지 정한다. 설비가 여러 대인 사이트가 생기면 `READINESS_EXTRA_SOURCES` 근사를 실제 로더와 맞춘다 | `lib/analytics/detectors/requirements.ts`, `lib/analytics/readiness/{matrix,registry,site}.ts`, `lib/data/readiness.ts` |
...
### 13.9 해결된 한계 (P3 후속, 2026-09-16)

§13.8에 "알려진 한계"로 적혀 있던 것 중 해결한 항목이다. 되돌아보기 쉽도록 **무엇이 틀렸는지 · 어떻게 고쳤는지 · 전후 수치 · 커밋**을 함께 남긴다. 각 항목의 "남은 부분"은 §13.8에 해당 행이 있다.

| 항목 | 무엇이 문제였나 | 어떻게 고쳤나 (전 → 후) | 커밋 | 남은 부분 |
|---|---|---|---|---|
| **저장용기 누설 검정 표준오차** | 판정값은 `최근 구간 가중 중앙값 − 기준 구간 중앙값`인데 표준오차를 `σ/√n최근`으로 썼다. 중앙값의 비효율(√(π/2))도, 기준 창의 불확실도도 빠져 있어 기준 12·최근 6구간에서 **실제 SE의 0.65배**였다. 명목 3σ 한쪽 검정(0.135%)이 실제로는 약 2σ(2.5%) 검정이었다 | SE = **√(π/2)·σ·√(1/n_eff최근 + 1/n기준)** (n_eff = (Σw)²/Σw², Kish 유효 표본수). CI 반폭 = max(1.96·SE, 최근 구간 부트스트랩 반폭), 기준 편향 보정 한도도 같은 규약(3 × √(π/2)·σ/√n기준)으로 맞췄다. 누설 0 합성 정지 구간 1000회 유의 판정 비율 **2.1% → 0.8%**. sim:eval 오탐 **0.0784 → 0.0055건/자산·월**(FP 198 → 14건), 정밀도 **0.066 → 0.533**, 탐지 지연 중앙값 **10.5 → 5일**, 크기 MAE 0.039 → 0.034. 검출력 확인: 실제 누설 0.4 kg/일 주입 200회 검출률 1.0(`tank-static-leak.test.ts` 기준은 ≥ 0.9, 검정 크기 기준은 ≤ 0.06). **탐지 임계값은 하나도 느슨하게 하지 않았다** | `d769a39` | 명목 0.135% vs 실측 0.8%의 잔여 차이(기준 σ 표본오차), 작은 누설(≤ 0.1 kg/일) 민감도 하락 — §13.8 |
| **저장용기 누설 게이트 재설정** | 오탐 게이트가 다른 탐지기와 같은 0.1건/자산·월이었고(안전 카테고리를 낼 수 있는데도), 최소 탐지 크기 게이트 임계가 스윕 최대(0.5 kg/일)여서 **어떤 값이 나와도 통과가 보장**됐다 | 오탐 게이트 `tank.static_leak`만 **0.1 → 0.02건/자산·월**(값 0.0055로 통과). 스윕에 0.15·0.25 kg/일을 추가해 0.1~0.3 구간을 다시 재고(0.1 → 2/3, 0.15 이상 3/3) 최소 탐지 크기 게이트를 **측정값 0.15 + 스윕 한 단계 = 0.2 kg/일**로 고정. 게이트 기준 크기의 근거는 §13.7에 항목으로 적었다. CI 축소 조합도 순번 8 → 6으로 바꿔 축소 51잡에서 게이트 38개 판정이 전체와 같음을 확인했다 | `8fb01b8`, `ab2f407` | 게이트 값은 시뮬레이터 기준이다 — 파일럿 실측으로 다시 본다 |
| **판별 체크 입력 3개 중 2개** | `tank.static_leak` 압력 교차 확인·`fc.blower_wear` 에어필터 막힘·`el.sec_rise` 퍼지 횟수가 **입력이 연결되지 않아 항상 '데이터없음'**이었다 | · 교차 확인: 체크 의미를 뒤집어 `pressure_drift`(센서 드리프트) → **`peer_pressure`**(같은 뱅크 다른 용기와 비교). 정지 보유 중에는 용기별 차단밸브가 닫혀 누설 용기만 압력이 떨어진다. **온도 보정 전 원 압력 기울기끼리** 빼서 뱅크 공통 냉각을 상쇄한다(보정한 손실률과 원 기울기를 섞어 비교했더니 정상 용기도 0.62 kg/일 손실로 보여 `unknown`이 나왔다 — 중간에 한 번 틀렸다가 고쳤다). 데모 SIM-B: **지지**, 이 용기 몫 1.021 kg/일 = 결합 누설률의 98.8%.<br>· 에어필터: "기준 기간 이전 교체"만 보던 규칙을 창 안(상승 이후 포함) 교체까지로 넓혔다. 데모: **지지**, 2026-09-05 교체 뒤 28.56% 회복.<br>· 퍼지: 로더는 연결했으나 카탈로그에 전해조 퍼지 카운터가 없어 데모에서는 여전히 데이터없음 | `d769a39`, `4c5f319` | 전해조 퍼지 카운터 부재, 판별 체크 정확도가 시뮬레이터로 검증되지 않음 — §13.8 |
| **준비도 주기 상한이 탐지기당 하나** | `minPeriodS`가 탐지기 단위여서, 스택 전압·전류 때문에 60초로 잡힌 탐지기의 스택 온도·운전시간·유량 적산까지 60초를 요구했다. 실제로는 필요 없는 요구라 SIM-B·SIM-C가 **52/55 '부분'** | `DetectorRequirements.metrics`를 `string[]` → `RequiredMetric = { key, maxPeriodS, optional? }`로 바꾸고 상한을 메트릭마다 선언했다(`FAST_S` 60초 = 전기적 순시값 7개, `SLOW_S` 300초 = 적산·온도·압력·상태·누적 카운터). 탐지기 14종 전부에 한 줄 근거 주석을 달고 `registry.test.ts`가 계약으로 고정한다. 준비도 **SIM-B·SIM-C 52/55 → 55/55**(SIM-A 41/41 유지). 함께 **권장 메트릭**(`optional`) 개념을 넣어 판별 체크·보조 축 입력이 빠진 것이 화면 `*`·CSV '누락 권장 메트릭' 열에 보이게 했다 — 확보 순위에는 넣지 않는다(없어도 탐지기가 돈다) | `6d17ec4`, `63513d6` | 상한 값 자체의 제조사 사양 검증, 완결성 기준 시각 — §13.8 |
| **조치 효과 검증이 탐지기와 다른 표본을 썼다** | 용량 조치 검증(`ess.capacity_ah`)이 전·후에서 **서로 다른 용량 산출 방식**을 섞을 수 있었다. 부분 사이클 사이트에서 전은 앵커(380 Ah)·후는 CC(396 Ah)면 **+16 Ah 가짜 개선**이 나온다 | `matched_before_after@1`에 표본 방식(method)을 넣고, 용량 지표는 탐지기와 같은 `CAPACITY_METHOD_ORDER`(앵커 > 휴지 앵커 > CC > SOC 변화)로 **전·후 양쪽이 bin당 3개·합계 5개를 채우는 첫 방식**을 골라 그 방식으로만 비교한다. 방식이 다르면 `insufficient_data`이고 방식별 표본 수를 `before_stats.methods`에 남긴다. 쓴 방식은 `before_stats`·`after_stats.method`와 리포트 팩(`method`·`methodLabel`, `report-planner@2 → @3`)에 남는다. 표본 규칙은 `CAPACITY_SAMPLE_RULE_DEFAULTS` 한 곳으로 모아 탐지와 검증이 갈라질 수 없게 했다. 부분 사이클 integration(하루 휴지 → 8 h 충전 → 휴지 → 8 h 방전, ΔSOC 30%p): 충전 세션 용량 3종이 전부 null인 조건에서 랙 교체 뒤 `improved`, 효과 **+20 Ah**(380 → 400), 방식 `rest_anchored` | `da3169a`, `35b4cf4`, `34514e7` | 앵커 이외 방식의 BMS SOC 의존 — §13.8 |
| **개발 의존성 취약점 8건** | `npm audit` high 5(brace-expansion·browserslist·flatted·js-yaml·minimatch) · moderate 2(@humanfs/node·ajv) · low 1(@babel/core). 운영 의존성(`--omit=dev`)은 원래 0건 | `npm audit fix`(`--force` 없음, 메이저 업그레이드 없음)로 **8건 → 0건**. `package.json`은 그대로이고 `package-lock.json`의 간접 의존성만 마이너·패치로 올라갔다. 적용 뒤 typecheck·lint·unit·integration·e2e·build를 다시 통과했다 | `34514e7` | 운영 전환 직전에 다시 확인한다 |

커버리지도 함께 올렸다(숫자만 맞춘 것이 아니라 실제 경로를 검증했다): `lib/analytics/pipeline/extract.ts` 라인 53.84% → **100%**(설비 종류 8종 라우팅·명판 오류 6종·추출 대상 아닌 6종), `lib/analytics/verification/before-after.ts` 라인 79.03% → **97.87%**(지표별 자격 조건·방식별 bin 키 경계). `lib/analytics` 전체 라인 98.46% → 99.15%.

**수치를 읽을 때 주의할 것 두 가지.**

- 검정 크기 **2.1% → 0.8%**(§13.9)는 저장소 테스트의 합성 정지 구간 설계(기준 12·최근 6구간, 구간당 24점, 구간 손실률 σ 0.08 kg/일)에서 나온 값이다. 독립 검증이 다른 합성 설계로 다시 재면 **2.4% → 1.5%**였다. 정정이 오탐을 줄인다는 방향과 "≤ 6%" 기준은 두 설계에서 모두 확인됐지만, 절대 수치는 합성 설계에 따라 달라진다. 현장 값은 파일럿에서 다시 잰다.
- 이 문서와 보고서가 말하는 **"열린 발견사항"은 상태값이 아니다.** `om.finding.status`에 `open`은 없고, `verified`·`dismissed`가 아닌 상태(`new`·`triaged`·`in_report`·`action_taken`·`reopened`)를 함께 부르는 말이다. 데모의 13건은 `new` 6건 + `in_report` 7건이다. SQL로 확인할 때 `status = 'open'`으로 조회하면 0건이 나온다.
