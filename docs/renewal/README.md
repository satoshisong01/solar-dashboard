...
커버리지도 함께 올렸다(숫자만 맞춘 것이 아니라 실제 경로를 검증했다): `lib/analytics/pipeline/extract.ts` 라인 53.84% → **100%**(설비 종류 8종 라우팅·명판 오류 6종·추출 대상 아닌 6종), `lib/analytics/verification/before-after.ts` 라인 79.03% → **97.82%**(지표별 자격 조건·방식별 bin 키 경계). `lib/analytics` 전체 라인 98.46% → 99.15%.
