...
| 판별 체크 입력 — 남은 것 | 입력 연결은 §13.9에서 했다. 남는 것은 둘이다.<br>· **전해조 퍼지 카운터가 카탈로그에 없다.** `purge.count`는 연료전지 FC1(애노드 퍼지)에만 매핑돼 있어 `el.sec_rise` 퍼지 체크는 데모 DB에서 계속 '데이터없음'이다. 로더는 전해조 설비·스택에 포인트가 생기면 바로 동작한다. 포인트를 먼저 추가하고 데이터가 없으면 `dq.gap_flatline`이 새 finding을 내므로, 추가는 수집이 시작되는 시점에 맞춘다.<br>· **메모리 평가(`lib/sim/eval`)에는 교차 확인·퍼지 입력을 넣지 않았다.** 두 체크는 판정(심각도·finding 수)에 영향이 없어 게이트에 쓰이지 않고, 넣으면 준비 결과 캐시가 무효가 되어 평가 시간만 는다. 따라서 **판별 체크의 지지·반박 정확도는 시뮬레이터로 검증되지 않았다** — 데모 DB 1건씩만 확인했다 | 전해조 퍼지 카운터를 데이터 계약에 넣을지 정하고, 넣으면 `metric_def`·포인트 매핑을 수집 개시와 함께 추가한다. 판별 체크 정확도는 파일럿 기각 사유(§8 P4)로 확인한다 | `lib/analysis/aux-inputs.ts`, `lib/analysis/tank-holds.ts`, `db/seed/templates-hydrogen.ts`, `lib/sim/eval/` |
...
| 카탈로그에 없는 메트릭 | 일 강수량(오염 복원은 맑은 날 PI 급상승으로 대신), 압축기 흡입 가스 온도(외기 온도로 대신), 전해조 퍼지 카운터(`el.sec_rise` 퍼지 체크가 데이터없음 — 준비도에 권장 메트릭 누락으로 보인다) | 현장에서 받을 수 있으면 `metric_def`에 추가하고 탐지기 입력을 바꾼다 | `db/seed/catalog.ts`, `lib/analytics/episodes/compressor.ts` |
| 데모에 남은 권장 메트릭 누락 | 준비도 화면·CSV에 `*`로 보이는 2건이다(준비 상태 자체는 유지). COMP1 `vibration.rms` — 카탈로그에는 있으나 시드에서 일부러 매핑하지 않았다(`comp.sec_rise` 판별 체크 ③이 계속 데이터없음). ELZ1/STACK1 `purge.count` — 카탈로그에 없다 | 벤더 협의에서 권장 메트릭 목록([부록 A.2](data-contract-draft.md))을 함께 넘긴다 | `lib/analytics/readiness/matrix.ts`, `db/seed/` |
