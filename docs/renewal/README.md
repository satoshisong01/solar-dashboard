...
|---|---|---|---|
| SOC 기반 용량 추정의 BMS 의존 | · 휴지 앵커·CC·SOC 변화 방식은 BMS SOC 재보정 품질에 의존한다.<br>· 시뮬레이터 SOC는 참값이라 재보정 오차가 없어 평가 결과가 실제보다 좋게 나온다.<br>· 용량 감소가 끝난 뒤 처음 나타난 bin은 그 감소를 보지 못한다.<br>· 용량 조치 효과 검증(`ess.capacity_ah`)도 탐지기와 같은 우선순위(앵커 > 휴지 앵커 > CC > SOC 변화)로 전·후 양쪽이 채워지는 첫 방식을 골라 그 방식으로만 비교한다(2026-09-16). 전은 앵커·후는 CC처럼 방식이 다르면 비교하지 않고 `insufficient_data`이고, 방식별 표본 수를 `before_stats.methods`에 남긴다. 쓴 방식은 `before_stats`·`after_stats.method`와 리포트 팩(`method`·`methodLabel`)에 남는다.<br>· 그래도 휴지 앵커·CC·SOC 변화는 모두 BMS SOC에 기댄다 — 앵커 이외의 방식으로 나온 "개선"은 SOC 재보정 변화와 구분되지 않는다 | 파일럿 BMS의 휴지 OCV 재보정 동작과 SOC 점프 빈도를 확인한다. 정기 용량시험 결과를 조치로 기록한다 | `lib/analytics/detectors/ess-capacity-samples.ts`, `lib/analytics/verification/before-after.ts` |
...
| 판별 체크 입력 (퍼지만 남음) | · `tank.static_leak` 압력 교차 확인과 `fc.blower_wear` 에어필터 막힘은 연결했다(§13.3). 데모 DB에서 각각 '지지'가 나온다.<br>· `el.sec_rise` 퍼지 횟수는 로더를 연결했지만 카탈로그에 전해조 퍼지 카운터 포인트가 없어(`purge.count`는 연료전지 FC1에만 있다) 데모 DB에서는 여전히 '데이터없음'이다. 전해조에 포인트를 추가하면 그 시점부터 값이 생긴다.<br>· 메모리 평가(`lib/sim/eval`)에는 교차 확인·퍼지 입력을 넣지 않았다. 두 체크는 판정(심각도·finding 수)에 영향이 없어 게이트에 쓰이지 않는다.<br>· 판별 체크 입력은 이제 탐지 준비도에 **권장 메트릭**으로 올라가 화면·CSV에서 빠진 것이 보인다(2026-09-16): `el.sec_rise` 정류기 효율·퍼지 횟수, `comp.sec_rise` 토출 온도·누설 감지 압력·진동·운전시간, `tank.static_leak` 하류 공급 압력, `pv.soiling_rate` GHI, `fc.voltage_decay` 블로워 전력, `h2chain.mass_balance_gap` 적산계·퍼지 횟수 | 전해조 퍼지 카운터를 데이터 계약에 넣을지 정하고, 넣으면 `metric_def`·포인트 매핑을 추가한다 | `lib/analysis/aux-inputs.ts`, `lib/analysis/tank-holds.ts`, `db/seed/templates-hydrogen.ts` |
...
| 준비도 주기·기준 시각 (주기는 해소) | · 주기 상한을 메트릭별로 나눴다(2026-09-16). 전기적 순시값 60초, 적산·온도·압력·상태·누적 카운터 300초. SIM-B·SIM-C 준비 52/55 → 55/55 — 스택 온도·누적 운전시간·유량 적산·전력량이 300초라 '부분'이던 `el.voltage_rise`·`el.sec_rise`·`fc.voltage_decay` 셀이 준비가 됐다.<br>· 권장 메트릭이 빠진 셀은 준비 상태를 유지하면서 `*`로만 표시한다. 데모에 남은 것은 COMP1 `vibration.rms`(시드에서 일부러 매핑하지 않음)와 ELZ1/STACK1 `purge.count`(카탈로그에 없음) 둘이다.<br>· 완결성은 '지금'부터 30일로 계산해, 수신이 멈춘 데모 데이터는 날이 지날수록 완결성이 떨어진다 | 메트릭별 상한 값은 현장 설비 사양이 오면 다시 본다(특히 DCIR용 랙 전압·전류는 2초 이하 권장). 완결성 기준 시각을 '마지막 수신'으로 바꿀지 정한다 | `lib/analytics/detectors/requirements.ts`, `lib/analytics/readiness/matrix.ts`, `lib/data/readiness.ts` |
...
| 개발 의존성 취약점 (해소) | `npm audit`(2026-09-16): 운영·개발 모두 **0건**.<br>· 이전 8건(high 5: brace-expansion·browserslist·flatted·js-yaml·minimatch / moderate 2: @humanfs/node·ajv / low 1: @babel/core)을 `npm audit fix`로 해결했다. `--force`를 쓰지 않았고 메이저 업그레이드도 없다.<br>· `package.json`은 그대로이고 `package-lock.json`의 간접 의존성만 마이너·패치로 올라갔다. 적용 뒤 typecheck·lint·unit·integration·e2e·build를 다시 통과했다 | 운영 전환 직전에 다시 `npm audit`을 확인한다 | `package-lock.json` |
